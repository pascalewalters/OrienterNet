...
second_floor_features = [
    feature
    for feature in floor["features"]
    if feature["properties"]["id"] == second_floor_id
]
...
# Point
with open(
    os.path.join(mvf_folder, "annotation", second_floor_id + ".geojson"), "r"
) as f:
...
with open(
    os.path.join(mvf_folder, "obstruction", second_floor_id + ".geojson"), "r"
) as f:
        ...
        output_dict["elements"].append(
            {
                ...
                "visible": True,
                "members": [
                    {"type": "node", "ref": i, "role": "outer"} for i in indexes
                ],
            }
        )
    idx += 1
