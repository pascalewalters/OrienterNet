...
def prepare_mvf_data(
    ...
    for floor in dataset.floors["features"]:
        ...
        geometries = {
            ...
            "nodes": [
                node
                for node in dataset.nodes["features"]
                if node["properties"]["floorId"] == floor_id
            ],
            "connections": [
                connection
                for connection in dataset.connections.values()
                if connection["properties"]["floorId"] == floor_id
            ],
            "windows": [
                window
                for window in dataset.windows["features"]
                if window["properties"]["floorId"] == floor_id
            ],
            "obstructions": [
                obstruction for obstruction in dataset.obstructions["features"]
            ],
