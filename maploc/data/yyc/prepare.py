...
def train_val_split(combined_geojson_path, output_path):
    ...
    for map_id, images in images_by_map.items():
        ...
        split_idx = int(
            len(images) * 0.95
        )  # bad split but maximize train for now, and can manually inspect model performance at first?
...
def prepare_osm(
    ...
    for feature in floor_features:
        ...
        if (
            coords[1] > osm_data["bounds"]["maxlat"]
            or coords[1] < osm_data["bounds"]["minlat"]
        ):
            print(
                f"Image: {feature['properties']['imageUrl']} LATITUDE: {coords[1]} outside of bounds: max: {osm_data['bounds']['maxlat']} min: {osm_data['bounds']['minlat']}"
            )
        if (
            coords[0] > osm_data["bounds"]["maxlon"]
            or coords[0] < osm_data["bounds"]["minlon"]
        ):
            print(
                f"Image: {feature['properties']['imageUrl']} LONGITUDE: {coords[0]} outside of bounds: max: {osm_data['bounds']['maxlon']} min: {osm_data['bounds']['minlon']}"
            )
        if (
            coords[1] > osm_data["bounds"]["maxlat"]
            or coords[1] < osm_data["bounds"]["minlat"]
            or coords[0] > osm_data["bounds"]["maxlon"]
            or coords[0] < osm_data["bounds"]["minlon"]
        ):
