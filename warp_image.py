...
osm_path = Path(
    "/home/kevinmeng/workspace/mappedin/VPS/OrienterNet/yyc_osms/f_64ff43e17107d168de589c1a_osm.json"
)

# Get ground truth from geojson file
geojson_path = Path(
    "/home/kevinmeng/workspace/mappedin/VPS/OrienterNet/datasets/YYC/combined-output.geojson"
)
...
tiler = TileManager.from_bbox(
    proj, bbox + 10, demo.config.data.pixel_per_meter, path=osm_path
)
...
plot_nodes(
    1, canvas.raster[1], fontsize=6, size=10
)  # canvas.raster[2] does not exist after areas were removed
...
if ground_truth:
    plot.points(
        [ground_truth["lat"], ground_truth["lon"]], "blue", name="ground truth", size=10
    )
    print(
        f"Ground truth: lat={ground_truth['lat']}, lon={ground_truth['lon']}, bearing={ground_truth['bearing']}"
    )
